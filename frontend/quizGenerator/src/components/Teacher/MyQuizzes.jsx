...
//       try {
//         const res = await axios.get('http://localhost:3141/api/v1/teacher/quizzes', { headers: {
...
//     if (!confirmed) return;
//     try {
//       await axios.delete(`http://localhost:3141/api/v1/teacher/quiz/${id}`, { headers: {
...
//                   {quiz.isScheduled ? (
//                     <span className="text-green-400">{new Date(quiz.scheduledAt).toLocaleString()}</span>
//                   ) : (
...
//                     <button
//                       onClick={() => navigate(`/teacher/finalresults/${quiz._id}`)}
...
const MyQuizzes = () => {
  ...
  const handleEditSchedule = (id) => {
    navigate(`/teacher/quiz/${id}`); // Navigates to quiz details/scheduling page
  };

  const handleViewResults = (id) => {
    navigate(`/teacher/finalresults/${id}`); // Navigates to results page
  };
